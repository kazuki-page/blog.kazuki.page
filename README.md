...
1 枚 0.4 秒かかるので、全記事ぶんを毎回作ると 3 分になる。
生成済みの PNG を `.ogp-cache/` に残し、**増えた記事のぶんだけ作る**。
CI では GitHub Actions のキャッシュで持ち越す。

使い回してよいかは、画像の見た目を決めるものをまとめた指紋で判断する。

| 指紋に含むもの | 変えたときに起きること |
| --- | --- |
| `src/lib/ogp.ts`（折り返しと配置） | 全て作り直し |
| `src/lib/ogp-render.ts`（描画） | 全て作り直し |
| `src/assets/ogp/`（背景・フォント） | 全て作り直し |
| satori / resvg のバージョン | 全て作り直し |
| 記事のタイトル | その記事だけ作り直し |

キャッシュのファイル名はタイトルのハッシュなので、タイトルを直せば別のファイルになる。
使われなくなったファイルはビルドのたびに掃除する。

### 全て作り直す

見た目を変えていないのに作り直したいとき（キャッシュを疑うとき）は次のようにする。

```bash
OGP_REGENERATE=1 npm run build
```

`.ogp-cache/` を消しても同じだが、`OGP_REGENERATE=1` なら生成結果は書き戻すので
次のビルドはまた速い。

CI では、Actions タブから手動実行するときに **「OGP 画像を全て作り直す」を選ぶ**。
このときキャッシュのキーが毎回変わるため、作り直した結果が確実に保存される。
